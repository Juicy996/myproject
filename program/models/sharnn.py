...
class SHARNN(nn.Module):
    def __init__(self, model_config, template):
        ...
        for idx in range(num_layers):
            rnn = True
            uta = True if idx == num_layers - 2 else False
            input(uta)
            self.blocks.append(Block(embed_dim, hidden_dim, self.num_heads, dropout=dropouth, rnn=rnn, residual=False, use_attn=uta))
    ...
    def forward(self, batch, embedding, extra_input, template, writer):
        ...
        total_length = len(e) + (len(mems[0]) if mems else 0)
        ...
        h = self.drop(h).transpose(0, 1)

        extra_output['mems'] = new_mems
        extra_output['hidden'] = new_hidden
        return h, extra_output
