import os, sys, time, torch
# import torch.nn as nn
from . import register_criterion, basic_criterion
# from torch.nn.modules.loss import _Loss



@register_criterion('triple_order')
class triple_order(torch.nn.Module):
    def __init__(self, hidden_dim, num_label):
        super().__init__()
        self.hidden_dim = hidden_dim
        self.num_label = num_label
        self.fnn = torch.nn.Linear(hidden_dim, num_label)
        self.lbd = 1.0
        self.sigma1 = torch.nn.Parameter(torch.FloatTensor(torch.tensor([0.5])))
        self.sigma2 = torch.nn.Parameter(torch.FloatTensor(torch.tensor([0.5])))

    def forward(self, rep, target, reduce = True, extra_input = None):
        source, target = extra_input['to_loss']
        logits = self.fnn(rep)
        logits = logits.reshape([-1, self.num_label])
        target = target.reshape(-1).long()

        predicts = torch.argmax(logits.detach(), dim = 1)
        corrects = torch.eq(predicts, target).float().sum()
        total = len(target)

        # loss = torch.nn.functional.nll_loss(logits, target, size_average = False, ignore_index = self.padding_idx, reduce = reduce)
        loss = torch.nn.functional.cross_entropy(logits, target)#, reduction = 'sum', ignore_index = self.padding_idx
        pens = self.pen(rep, predicts, source, target)# * 0.000001
        # input(f'loss = {loss}, pens = {pens}')
        loss = loss / (self.sigma1 ** 2) + pens / (2 * self.sigma2 ** 2) + torch.log(self.sigma1) + torch.log(self.sigma2)

        ret = {}
        ret['loss'] = loss.unsqueeze(0)
        ret['correct'] = corrects.detach().unsqueeze(0)
        ret['total'] = torch.tensor(total, device = loss.device, dtype = loss.dtype).detach().unsqueeze(0)
        ret['loss_detach'] = loss.detach().unsqueeze(0)

        return ret

    def pen(self, rep, predict, src, targets):
        src = torch.mean(src, dim = 1)
        
        cen_rep, cen_src = [], []
        for i in range(self.num_label):
            cen_rep.append(torch.mean(rep[torch.eq(predict, i)], dim = 0) if i in predict else torch.zeros([self.hidden_dim], device = rep.device))
            cen_src.append(torch.mean(src[torch.eq(targets, i)], dim = 0) if i in targets else torch.zeros([self.hidden_dim], device = rep.device))

        cen_rep = torch.cat(cen_rep).reshape([-1, self.hidden_dim])
        cen_src = torch.cat(cen_src).reshape([-1, self.hidden_dim])

        k_ = self.soe(cen_src)
        s_ = self.soe(cen_rep)

        ret = (k_ - s_) ** 2
        ret = torch.mean(ret)
        return ret

    def soe(self, centers):
        # b1 = a.unsqueeze(0).repeat(a.shape[0], 1, 1)
        # b2 = a.unsqueeze(1).repeat(1, a.shape[0], 1)
        # b3 = a.unsqueeze(1).repeat(1, a.shape[0], 1)

        # ab = (b1 - b2) **2
        # ac = (b1 - b3) **2

        # ab = ab.unsqueeze(2).repeat(1, 1, a.shape[0], 1)
        # ac = ac.unsqueeze(1).repeat(1, a.shape[0], 1, 1)
        # br = ab - ac + lbd
        b1 = centers.unsqueeze(0).unsqueeze(0).repeat(centers.shape[0], centers.shape[0], 1, 1)
        b2 = centers.unsqueeze(0).unsqueeze(2).repeat(centers.shape[0], 1, centers.shape[0], 1)
        b3 = centers.unsqueeze(1).unsqueeze(2).repeat(1, centers.shape[0], centers.shape[0], 1)

        # Euclidean Distance
        br = torch.mean((b1 - b2) ** 2 - (b1 - b3) ** 2 + self.lbd, dim = -1)

        # Kullback-Leibler Divergence
        # br = torch.sum(b1 * torch.log(b1 / (b2 + 1e-12)), dim = -1) - torch.sum(b1 * torch.log(b1 / (b3 + 1e-12)), dim = -1)

        # cos9ine similarity
        # br = (self.cos_sim(b1, b2) - self.cos_sim(b1, b3)) ** 2

        br = br.permute(2,1,0)

        return br
    
    def cos_sim(self, a, b):
        u = a * b
        d = (a ** 2) ** 0.5 + (b ** 2) ** 0.5
        ret = torch.sum(u / d + 1e-12, dim = -1)
        return ret

    @classmethod
    def setup_criterion(cls):
        return cls


















 
    