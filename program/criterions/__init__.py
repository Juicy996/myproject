...
class basic_criterion(torch.nn.Module):
    def __init__(self, criterion_name, hidden_dim, num_label):
        super().__init__()
        self.criterion_name = criterion_name
        self.hidden_dim = hidden_dim
        self.num_label = num_label
        self.fnn = torch.nn.Sequential(
            torch.nn.Linear(in_features  = self.hidden_dim, out_features  = self.num_label)
        ) 
        print('Criterion [{}] has beed built.'.format(self.criterion_name))
    
    @classmethod
    def setup_criterion(cls):
        raise NotImplementedError('Init basic is not allowed.')

class ProjectedAdaptiveLogSoftmax(torch.nn.Module):
...
def load_criterion(criterion_name):
    return criterion_dict[criterion_name].setup_criterion()

def register_criterion(name):
    def register_criterion(cls):
        if name in criterion_dict:
            raise ValueError('Cannot register duplicate criterion ({})'.format(name))
        criterion_dict[name] = cls
        return cls
...
for file in os.listdir(os.path.dirname(__file__)):
    if file.endswith('.py') and not file.startswith('_'):
        ...
        importlib.import_module('program.criterions.' + module)