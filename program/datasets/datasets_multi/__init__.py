...
class basic_multi_dataset(basic_dataset):
    ...
    def load_dataset(self, file_trn: str, file_val: str = None, file_tst: str = None):
        self.dict_src, self.dict_wiz = self.build_dict([file_trn, file_val, file_tst])
        ...
        self.trn = swt_dataset(source_trn, wizard_trn, target_trn)
        self.val = swt_dataset(source_val, wizard_val, target_val)
        self.tst = swt_dataset(source_tst, wizard_tst, target_tst)

        if 'glove' in self.pretrained:
            self.embedding_matrix_src, self.missing_src = self.get_embedding_matrix(self.dict_src) 
            self.embedding_matrix_wiz, self.missing_wiz = self.get_embedding_matrix(self.dict_wiz) 
        else:
            self.embedding_matrix_src, self.embedding_matrix_wiz = None, None
            self.missing_src, self.missing_wiz = 0, 0
    ...
    def print_self(self):
        print(f'The length of dictionary is {len(self.dict_src)} and {len(self.dict_wiz)}')
